...
var _stripTypeScriptTypes = __webpack_require__("./src/stripTypeScriptTypes.ts");
...
class ScriptTransformer {
  ...
  _buildCacheKeyFromFileInfo(fileData, filename, transformOptions, transformerCacheKey, willStripTypes) {
    ...
    return (0, _nodeCrypto().createHash)('sha1').update(fileData).update('\0', 'utf8').update(transformOptions.configString).update('\0', 'utf8').update(transformOptions.instrument ? 'instrument' : '').update('\0', 'utf8').update(callerSupport(transformOptions)).update('\0', 'utf8').update(filename).update('\0', 'utf8')
    ...
    .update(willStripTypes ? process.version : '').update('\0', 'utf8').update(CACHE_VERSION).digest('hex').slice(0, 32);
  }
  ...
  _getCacheKey(fileData, filename, options) {
    ...
    return this._buildCacheKeyFromFileInfo(fileData, filename, transformOptions, transformerCacheKey, transformer == null && this._shouldStripTypes(filename));
  }
  ...
  _buildTransformResult(filename, cacheFilePath, content, transformer, shouldCallTransform, options, processed, sourceMapPath) {
    ...
    if (transformer && shouldCallTransform) {
      ...
      transformed = {
        code: (0, _stripTypeScriptTypes.stripTypes)(content, filename),
        map: null
      };
    }
  ...
  async _transformAndBuildScriptAsync(filename, options, transformOptions, fileSource) {
    ...
    const willTransform = isInternalModule !== true && (transformOptions.instrument || this.shouldTransform(filename) || this._shouldStripTypes(filename));
    try {
      if (willTransform) {
  ...
  _shouldStripTypes(filename) {
    if (!(0, _stripTypeScriptTypes.canStripTypes)(filename)) {
      return false;
    }
    const ignoreRegexp = this._cache.ignorePatternsRegExp;
...
exports.canStripTypes = canStripTypes;
exports.stripTypes = stripTypes;
exports.supportsTypeStripping = void 0;
function nodeModule() {
...
const nodeStripTypeScriptTypes = nodeModule().stripTypeScriptTypes;
const supportsTypeStripping = exports.supportsTypeStripping = typeof nodeStripTypeScriptTypes === 'function';
...
function burnExperimentalWarning(strip) {
  warningBurnt = true;
...
function canStripTypes(filename) {
  return supportsTypeStripping && TYPESCRIPT_EXTENSIONS.has(path().extname(filename));
}
function stripTypes(code, filename) {
  (0, _jestUtil().invariant)(nodeStripTypeScriptTypes, 'Node.js cannot strip TypeScript types');
  if (!warningBurnt) {
    burnExperimentalWarning(nodeStripTypeScriptTypes);
  }
  try {
    return nodeStripTypeScriptTypes(code, {
      mode: 'strip'
    });
  } catch (error) {
    const reason = (0, _jestUtil().isError)(error) ? error.message : String(error);
    throw new Error(`jest: failed to strip TypeScript types from ${filename}\n\n${reason}\n\n` + 'Node.js only erases type annotations - it cannot compile TypeScript features that emit code, such as `enum`, `namespace` and parameter properties. Configure a `transform` using `@babel/preset-typescript` or `ts-jest` to handle this file.', {
      cause: error
    });
  }
}
