...
const interpolateVariables = (title, template, index) => {
  ...
  const keys = Object.keys(template).filter(key => key.length > 0).sort((a, b) => b.length - a.length);
  if (keys.length === 0) {
    return title.replace('$#', `${index}`);
  }
  const alternation = keys.map(_jestRegexUtil().escapeStrForRegex).join('|');
  return title.replaceAll(new RegExp(`\\$(${alternation})[.\\w]*`, 'g'), match => {
